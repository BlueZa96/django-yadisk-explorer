...
function updateFileExplorer() {
    ...
    const filteredFiles = selectedFileType
        ? state.files.filter(file => file.extension && file.extension.toLowerCase() === selectedFileType.toLowerCase())
        : state.files;

    if (state.currentPath) addNavigationButtons(explorerItems);
    state.folders.forEach(folder => addFolderItem(explorerItems, folder));

    if (filteredFiles.length) {
        filteredFiles.forEach(file => addFileItem(explorerItems, file));
    } else if (!state.folders.length && !filteredFiles.length) {
        addEmptyFolderMessage(explorerItems);
    } else if (!filteredFiles.length && selectedFileType) {
        addNoMatchingFilesMessage(explorerItems);
    }
}
...
function addFileItem(container, file) {
    container.innerHTML += `
        <div class="list-group-item d-flex align-items-center flex-grow-1">
            <input type="checkbox" value="${file.file}" data-name="${file.name}" data-type="file" class="item-checkbox file-checkbox me-2">
...
function addEmptyFolderMessage(container) {
    container.innerHTML += '<div class="list-group-item">Папка пуста</div>';
}

function addNoMatchingFilesMessage(container) {
    const selectedType = document.getElementById('file_type').value;
    container.innerHTML += `<div class="list-group-item">Нет файлов с расширением "${selectedType}"</div>`;
}
...
function updateFileTypeFilter() {
    ...
    const extensions = [...new Set(state.files
        .map(f => f.extension)
        .filter(Boolean)
    ...
    extensions.forEach(ext => {
        typeSelect.innerHTML += `<option value="${ext}">${ext}</option>`;
    });
}

function toggleUIElements(show) {
    ['file-type-filter', 'download-buttons', 'current_folder'].forEach(id => {
